...
export async function getServerSideProps({ res, query }) {
  ...
  const T = {
    en: {
      ...
      aFeatures:['Equipped kitchen: Vitamix, reverse-osmosis water filter, electric cooktop','1 bedroom (functional and cool)','Large open living/kitchen space','2 A/C units + natural ventilation (often not needed)'],
    ...
    es: {
      ...
      aFeatures:['Cocina equipada: Vitamix, filtro de ósmosis y estufa eléctrica','1 recámara (funcional y fresca)','Espacio abierto sala/comedor/cocina','2 A/C + ventilación natural (muchos días no hacen falta)'],
    ...
    fr: {
      ...
      aFeatures:['Cuisine équipée : Vitamix, filtre à osmose inverse, plaque électrique','1 chambre (fonctionnelle et fraîche)','Grand espace ouvert salon/cuisine','2 climatiseurs + ventilation naturelle'],
  ...
  const PHOTO_ALT = {
    en: {
      ...
      "cocina": "Kitchen with Vitamix, reverse-osmosis water filter and electric cooktop",
    ...
    es: {
      ...
      "cocina": "Cocina con Vitamix, filtro de ósmosis y estufa eléctrica",
    ...
    fr: {
      ...
      "cocina": "Cuisine avec Vitamix, filtre à osmose inverse et plaque électrique",
...
<div class="relative h-[74vh] min-h-[520px] w-full overflow-hidden">
  ...
  <div class="absolute inset-0 mx-auto flex max-w-6xl items-end px-4 pb-6">
    ...
    <div class="md:hidden glass shadow-soft w-full rounded-2xl p-4">
      <p class="mb-1 inline-flex items-center rounded-full bg-emerald-50 px-2 py-0.5 text-xs font-semibold text-emerald-700">${d.badge}</p>
      <h1 class="text-2xl font-semibold tracking-tight">Casa Brasil Terrace</h1>
      <p class="mt-1 text-sm text-zinc-600">${d.heroSub}</p>
      <a class="mt-3 inline-flex w-full items-center justify-center rounded-xl bg-emerald-600 px-4 py-2.5 text-sm font-semibold text-white" href="#contact">${d.cta}</a>
    </div>

    <!-- DESKTOP: full card -->
    <div class="hidden md:block glass shadow-soft w-full rounded-3xl p-7">
      <div class="flex gap-4 items-end justify-between">
        <div class="max-w-2xl">
          ...
          <h1 class="text-5xl font-semibold tracking-tight">Casa Brasil Terrace</h1>
          <p class="mt-3 text-zinc-700 text-lg">${d.heroDesc}</p>
        ...
        <div class="flex flex-col gap-2 items-end flex-shrink-0">
          <a class="inline-flex items-center justify-center rounded-2xl bg-emerald-600 px-5 py-3 text-sm font-semibold text-white hover:bg-emerald-700" href="#contact">${d.cta}</a>
          <a class="inline-flex items-center justify-center rounded-2xl bg-white px-5 py-3 text-sm font-semibold text-zinc-900 border border-zinc-200 hover:bg-zinc-50" href="#location">${d.heroLink}</a>
          <p class="text-xs text-zinc-500 text-right">${d.heroSub}</p>
        </div>
      </div>
    </div>
